...
The fixed frame page proxies immich-kiosk through the controller origin, so Home Assistant next, previous, play/pause, reload, and kiosk video mute controls can be bridged into the renderer iframe. Renderer controls also include a profile-aware **Custom CSS Class** text entity, sent to immich-kiosk as `custom_css_class`; enter one class or a whitespace-separated class list, and the controller normalizes it for the renderer URL while saving the full supplied value with the profile. For frames running FreeKiosk, set the device remote-control type to `freekiosk` and enter the FreeKiosk REST API URL, for example `http://192.168.1.160:8080`, to enable screen on, screen off, direct display brightness, direct media volume, Android device mute toggle, ambient light status, auto-brightness status, an accelerometer-backed X/Y orientation hint, and iframe-command fallback when no frame browser is connected. immich-kiosk video mute is available as a stateful switch for the kiosk video player and as a press button for quick toggles; use Android device mute for reliable frame-wide audio state.
