...
The fixed frame page proxies immich-kiosk through the controller origin, so Home Assistant next, previous, play/pause, reload, and kiosk mute buttons can be bridged into the renderer iframe. For frames running FreeKiosk, set the device remote-control type to `freekiosk` and enter the FreeKiosk REST API URL, for example `http://192.168.1.160:8080`, to enable screen on, screen off, volume up, volume down, Android device mute toggle, and iframe-command fallback when no frame browser is connected.
