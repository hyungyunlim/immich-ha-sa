...
The fixed frame page proxies immich-kiosk through the controller origin, so Home Assistant next, previous, play/pause, reload, and kiosk video mute controls can be bridged into the renderer iframe. For frames running FreeKiosk, set the device remote-control type to `freekiosk` and enter the FreeKiosk REST API URL, for example `http://192.168.1.160:8080`, to enable screen on, screen off, direct display brightness, direct media volume, Android device mute toggle, ambient light status, auto-brightness status, and iframe-command fallback when no frame browser is connected. immich-kiosk video mute is available as a stateful switch for the kiosk video player and as a press button for quick toggles; use Android device mute for reliable frame-wide audio state.
