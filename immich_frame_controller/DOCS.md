...
Kiosk Video Mute is exposed as both a Home Assistant switch and a press button. The switch sends explicit `mute-on`/`mute-off` commands and uses immich-kiosk's `window.immichKiosk.video.setMuted(...)` API when available, then falls back to the legacy menu/video behavior. The button keeps the old quick-toggle behavior.
...
The Home Assistant Kiosk Video Mute switch controls immich-kiosk video mute, not Android device volume. It tracks the controller's desired kiosk video mute state; for reliable frame-wide audio state, use Device Mute Toggle instead because it sends Android `KEYCODE_VOLUME_MUTE` through FreeKiosk.
...
- Layout and display: `layout`, `imageFit`, `backgroundBlur`, `backgroundBlurAmount`, `fontSize`, `frameless`, `showArchived`, `customCssClass` (sent to immich-kiosk as `custom_css_class` and saved in profiles).
