...
export function buildRendererUrl(
  ...
  if (state.excludeVideosOver > 0) {
    url.searchParams.set('exclude_videos_over', String(state.excludeVideosOver));
  }
  ...
  url.searchParams.set('ifc_description_scroll_speed', String(state.imageDescriptionScrollSpeed));
